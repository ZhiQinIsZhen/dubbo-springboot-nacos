...
@ConditionalOnProperty(prefix = "request.limit", name = "model", havingValue = "caffeine")
public class LimitCaffeineAspect extends AbstractLimitService implements Ordered {

    private static LoadingCache<String, RateLimiter> caches = Caffeine.newBuilder()
            .maximumSize(100000)
            .initialCapacity(1000)
            .expireAfterWrite(5, TimeUnit.MINUTES)
            .build(key -> RateLimiter.create(getCount()));

    public LimitCaffeineAspect() {
        log.info("bean LimitCaffeineAspect init ...");
    }
